...
(function () {
  ...
  async function video(guide, steps, opts) {
    ...
    // 1080p, not 720p: these are dense dashboard screenshots, and the source
    // bitmap is retina-sized. At 720p the UI text was already illegible before
    ...
    if (narrationOn) {
      tl.segs.forEach((s) => {
        ...
        src.buffer = buf;
        src.connect(dest);
        src.connect(audioCtx.destination); // audible while it renders
    ...
    // switched away. A timer still fires (throttled to ~1/s), so the slides keep
    // advancing — choppy, but the right pictures in the right places.
    ...
    download(blob, safeName(guide.title) + ".webm");
    prog(1, "Done");
  }
